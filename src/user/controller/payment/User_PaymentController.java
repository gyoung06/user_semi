...
//		insert into orders values(ORDERS_seq.nextval,sysdate,'허지영','01052199755','경기 안양시 동안구 관평로 257경기 안양시 동안구 관양동 1586-5123호 (관양동)','15201',0,0,0,'N','N',null,'1234');
		...
		int ori = 0;
		if (saveDir.length == 3) {
			ori = ordersdao.buyproduct(saveDir[0], saveDir[2], saveDir[1], id);
		} else if (newDir.length == 3) {
			ori = ordersdao.buyproduct(newDir[0], newDir[2], newDir[1], id);
		} else if (upDir.length == 3) {
			ori = ordersdao.buyproduct(upDir[0], upDir[2], upDir[1], id);
		}
		if (ori <= 1) {
...
//			int n = ordersdetaildao.buyproduct(orColor[i], orSize[i], amount[i], allPay, payment, orid, pid);
