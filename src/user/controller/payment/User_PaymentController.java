...
//		insert into orders values(ORDERS_seq.nextval,sysdate,'허지영','01052199755','경기 안양시 동안구 관평로 257경기 안양시 동안구 관양동 1586-5123호 (관양동)','15201',0,0,0,'N','N',null,'1234');
		...
		int ori = 0;
		if (saveDir.length == 3) {
			ori = ordersdao.buyproduct(seq,saveDir[0], saveDir[2], saveDir[1], allSum, allSum - useMil, id);
		} else if (newDir.length == 3) {
			ori = ordersdao.buyproduct(seq,newDir[0], newDir[2], newDir[1], allSum, allSum - useMil, id);
		} else if (upDir.length == 3) {
			ori = ordersdao.buyproduct(seq,upDir[0], upDir[2], upDir[1], allSum, allSum - useMil, id);
		}
		if (ori <= 1) {
...
// insert into order_detail(odid,odcolor,odsize,odcount,odmileage,orid,pid) values(1,'레드','s',1,0,18000,18000,1,1);
		...
		for (int i = 0; i < leng; i++) {
			odi = ordersdetaildao.buyproduct(orColor[i], orSize[i],Integer.parseInt(amount[i]), seq, pid);
			if (odi <= 1) {
...
//		update product set psell = psell+(select odcount from order_detail where pid=1) where pid=1;
		...
		int upi = userproductdao.psellplus(pid);
		if (upi <= 1) {
...
//		update stock set samount= samount-(select odcount from order_detail where pid=1) where sid=(select sid from stock where sname=(select sname from stock where sid=(select sid from product where pid=1)) and scolor='레드' and ssize='S');
		...
		for (int i = 0; i < leng; i++) {
			int usi = userstockdao.sellamount(pid, orSize[i], orColor[i]);
			if(usi<=1) {
		...
		int procuctMil = Integer.parseInt(req.getParameter("addMileage"));
		String per = req.getParameter("per");
		int userMil = allSum*(Integer.parseInt(per))/100;
		int addMil = procuctMil+userMil;
